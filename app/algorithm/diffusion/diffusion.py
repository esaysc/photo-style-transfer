...
class WeightedLoss(nn.Module):
    ...
    def forward(self, pred, target, weights=1.0):
        ...
        WeightedLoss = (weights * loss).mean()
        return WeightedLoss
...
class SinisoidalPosEmb(nn.Module):
...
class MLP(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, device, t_dim=16):
        ...
        self.device = device
        self.time_mip = nn.Sequential(
            SinisoidalPosEmb(t_dim),
...
class DiffusionModel(nn.Module):
    def __init__(self, loss_type, beta_schedule="linear", clip_denoised=True, predict_epsilon=True, **kwargs):
      ...
      # alphas_cumprod_prev = torch.cat([torch.tensor([1.], device=self.device), alphas_cumprod[:-1]])
      ...
      self.register_buffer('sqrt_recipm_alphas_cumprod', torch.sqrt(1. / alphas_cumprod - 1))

      self.register_buffer('posterior_mean_coef1', betas * torch.sqrt(alphas_cumprod_prev) / (1. - alphas_cumprod))
      self.register_buffer('posterior_mean_coef2', (1. - alphas_cumprod_prev) * torch.sqrt(alphas) / (1. - alphas_cumprod))

      self.lose_fn = Losses[loss_type]()

    def q_posterior(self, x_start, x, t):
        posterior_mean = extract(self.posterior_mean_coef1, t, x.shape) * x_start + extract(self.posterior_mean_coef2, t, x.shape) * x
    ...
    def p_sample(self, x, t, s):
        ...
        model_mean, model_log_var = self.p_mean_variance(x, t, s)
    ...
    def q_sample(self, x_start, t, noise):
        ...
        return sample
    def p_losses(self, x_state, state, t, weights=1.0):
        noise = torch.randn_like(x_state)
        x_noisy = self.q_sample(x_state, t, noise)
        x_recon = self.model(x_noisy, t, state)
        # x_recon.clamp_(-1, 1)
        loss = self.lose_fn(x_recon, noise, weights)
        return loss
...
if __name__ == '__main__':
    ...
    model = DiffusionModel('l2', obs_dim=11, act_dim=2, hidden_dim=256, T=100, device=device)
    action = model(state)

    loss = model.loss(x, state)
    print(f'action shape: {action.shape}, loss: {loss.item()}')
