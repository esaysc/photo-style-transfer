import torch.nn as nn
import torch.nn.functional as F

class resBlock(nn.Module):
  def __init__(self, in_channel):
    super(resBlock, self).__init__()

    conv_block = [
        nn.ReflectionPad2d(1),
        nn.Conv2d(in_channel, in_channel, 3),
        nn.InstanceNorm2d(in_channel),
        nn.ReLU(inplace=True),
        nn.ReflectionPad2d(1),
        nn.Conv2d(in_channel, in_channel, 3),
        nn.InstanceNorm2d(in_channel),
    ]
    self.conv_block = nn.Sequential(*conv_block)
  def forward(self, x):
    return x + self.conv_block(x)
class Generator(nn.Module):
  def __init__(self, in_channel=3, out_channel=64):
    super(Generator, self).__init__()

    net = [
      nn.ReflectionPad2d(in_channel),
      nn.Conv2d(in_channel, out_channel, 7),
      nn.InstanceNorm2d(out_channel),
      nn.ReLU(inplace=True),
    ]

    # downsample
    for _ in range(2):
      net += [
        nn.Conv2d(out_channel, out_channel*2, 3, stride=2, padding=1),
        nn.InstanceNorm2d(out_channel*2),
        nn.ReLU(inplace=True),
      ]
      out_channel *= 2
    for _ in range(9):
      net += [resBlock(out_channel)]

    # upsampling
    for _ in range(2):
      net += [
        nn.ConvTranspose2d(out_channel, out_channel//2, 3, stride=2, padding=1, output_padding=1),
        nn.InstanceNorm2d(out_channel//2),
        nn.ReLU(inplace=True),
      ]
      out_channel //= 2
    net += [
      nn.ReflectionPad2d(in_channel),
      nn.Conv2d(out_channel, in_channel, 7),
      nn.Tanh(),
    ]

    self.net = nn.Sequential(*net)

  def forward(self, x):
    return self.net(x)
  
class Discriminator(nn.Module):
  def __init__(self, in_channel=3, out_channel=64):
    super(Discriminator, self).__init__()

    net = [
      nn.Conv2d(in_channel, out_channel, 4, stride=2, padding=1),
      nn.LeakyReLU(0.2, inplace=True),
    ]

    for _ in range(3):
      net += [
        nn.Conv2d(out_channel, out_channel*2, 4, stride=2, padding=1),
        nn.InstanceNorm2d(out_channel*2),
        nn.LeakyReLU(0.2, inplace=True),
      ]
      out_channel *= 2

    net += [
      nn.Conv2d(out_channel, 1, 4, padding=1),
    ]

    self.net = nn.Sequential(*net)

  def forward(self, x):
    x = self.net(x)
    return F.avg_pool2d(x, x.size()[2:]).view(x.size()[0], -1)
  

if __name__ == '__main__':
  G = Generator()
  D = Discriminator()
  import torch
  input_tensor = torch.ones((1, 3, 256, 256), dtype=torch.float)
  output = G(input_tensor)
  print(output.size())
  output = D(input_tensor)
  print(output.size())
  